...
export class Indexer implements IndexerInterface {
  ...
  constructor (serverConfig: ServerConfig, db: DatabaseInterface, clients: Clients, ethProvider: BaseProvider, jobQueue: JobQueue) {
    ...
    const { abi: AzimuthABI, storageLayout: AzimuthStorageLayout } = AzimuthArtifacts;

    assert(AzimuthABI);
    this._abiMap.set(KIND_AZIMUTH, AzimuthABI);
    assert(AzimuthStorageLayout);
    this._storageLayoutMap.set(KIND_AZIMUTH, AzimuthStorageLayout);
  ...
  async _owner (blockHash: string, contractAddress: string, diff = false): Promise<ValueResult> {
    ...
    const storageLayout = this._storageLayoutMap.get(KIND_AZIMUTH);
    ...
    await this._db._saveOwner({ blockHash, blockNumber, contractAddress, value: result.value, proof: JSONbigNative.stringify(result.proof) });
  ...
  async operators (blockHash: string, contractAddress: string, key0: string, key1: string, diff = false): Promise<ValueResult> {
    const entity = await this._db.getOperators({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('operators: db hit.');
    ...
    log('operators: db miss, fetching from upstream server');
    ...
    await this._db.saveOperators({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'operators', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async sponsoringIndexes (blockHash: string, contractAddress: string, key0: number, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getSponsoringIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('sponsoringIndexes: db hit.');
    ...
    log('sponsoringIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveSponsoringIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'sponsoringIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async escapeRequestsIndexes (blockHash: string, contractAddress: string, key0: number, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getEscapeRequestsIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('escapeRequestsIndexes: db hit.');
    ...
    log('escapeRequestsIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveEscapeRequestsIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'escapeRequestsIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async pointOwnerIndexes (blockHash: string, contractAddress: string, key0: string, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getPointOwnerIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('pointOwnerIndexes: db hit.');
    ...
    log('pointOwnerIndexes: db miss, fetching from upstream server');
    ...
    await this._db.savePointOwnerIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'pointOwnerIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async managerForIndexes (blockHash: string, contractAddress: string, key0: string, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getManagerForIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('managerForIndexes: db hit.');
    ...
    log('managerForIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveManagerForIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'managerForIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async spawningForIndexes (blockHash: string, contractAddress: string, key0: string, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getSpawningForIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('spawningForIndexes: db hit.');
    ...
    log('spawningForIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveSpawningForIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'spawningForIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async votingForIndexes (blockHash: string, contractAddress: string, key0: string, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getVotingForIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('votingForIndexes: db hit.');
    ...
    log('votingForIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveVotingForIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'votingForIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
  ...
  async transferringForIndexes (blockHash: string, contractAddress: string, key0: string, key1: number, diff = false): Promise<ValueResult> {
    const entity = await this._db.getTransferringForIndexes({ blockHash, contractAddress, key0, key1 });
    if (entity) {
      log('transferringForIndexes: db hit.');
    ...
    log('transferringForIndexes: db miss, fetching from upstream server');
    ...
    await this._db.saveTransferringForIndexes({ blockHash, blockNumber, contractAddress, key0, key1, value: result.value, proof: JSONbigNative.stringify(result.proof) });

    if (diff) {
      const stateUpdate = updateStateForMappingType({}, 'transferringForIndexes', [key0.toString(), key1.toString()], result.value.toString());
      await this.createDiffStaged(contractAddress, blockHash, stateUpdate);
    }
