...
* If the watcher is an `active` watcher:
  ...
  ```
  createdb naive-watcher-job-queue
  ```

  ```
  postgres@tesla:~$ psql -U postgres -h localhost naive-watcher-job-queue
  Password for user postgres:
  psql (12.7 (Ubuntu 12.7-1.pgdg18.04+1))
  SSL connection (protocol: TLSv1.3, cipher: TLS_AES_256_GCM_SHA384, bits: 256, compression: off)
  Type "help" for help.

  naive-watcher-job-queue=# CREATE EXTENSION pgcrypto;
  CREATE EXTENSION
  naive-watcher-job-queue=# exit
  ```
