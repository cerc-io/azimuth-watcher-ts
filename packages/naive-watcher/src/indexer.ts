...
import NaiveArtifacts from './artifacts/Naive.json';
...
export class Indexer implements IndexerInterface {
  ...
  constructor (
    ...
    const { abi: NaiveABI } = NaiveArtifacts;

    assert(NaiveABI);
    this._abiMap.set(KIND_NAIVE, NaiveABI);

    const NaiveContractInterface = new ethers.utils.Interface(NaiveABI);
    this._contractMap.set(KIND_NAIVE, NaiveContractInterface);

    const NaiveEventSignatures = Object.values(NaiveContractInterface.events).map(value => {
      return NaiveContractInterface.getEventTopic(value);
    });
    this.eventSignaturesMap.set(KIND_NAIVE, NaiveEventSignatures);
  }
  ...
  async naiveBatch (blockHash: string, blockNumber: number, contractAddress: string, index: number, rawBatchData: string): Promise<ValueResult> {
    // Look for the batch in the db first
    const entity = await this._db.getBatch({ blockHash, contractAddress, index });
    if (entity) {
      log('getBatch: found in db');
      return {
        value: entity,
    ...
    log('getBatch: db miss, writing it to db');
    const batch: DeepPartial<Batch> = { blockHash, blockNumber, contractAddress, index, rawBatchData };
    await this._db.saveBatch(batch);
  ...
  async getFullTransaction (txHash: string, blockNumber: number): Promise<EthFullTransaction> {
    return this._ethClient.getFullTransaction(txHash, blockNumber);
  }
}
