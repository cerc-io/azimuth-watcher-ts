...
export class Database implements DatabaseInterface {
  ...
  async getBatch ({ blockHash, contractAddress, index }: { blockHash: string, contractAddress: string, index: number }): Promise<Batch | undefined> {
    return this._conn.getRepository(Batch)
      .findOne({
        blockHash,
        contractAddress,
        index
  ...
  async saveBatch ({ blockHash, blockNumber, contractAddress, index, rawBatchData, proof }: DeepPartial<Batch>): Promise<Batch> {
    const repo = this._conn.getRepository(Batch);
    const entity = repo.create({ blockHash, blockNumber, contractAddress, index, rawBatchData, proof });
