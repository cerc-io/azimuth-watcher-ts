...
export class Indexer implements IndexerInterface {
  ...
  async getKeys (blockHash: string, contractAddress: string, _point: number): Promise<ValueResult> {
    ...
    const contract = new ethers.Contract(contractAddress, abi, this._ethProvider);
    const value = await contract.getKeys(_point, { blockTag: blockHash });

    const result: ValueResult = { value };

    await this._db.saveGetKeys({ blockHash, blockNumber, contractAddress, _point, value: result.value, proof: JSONbigNative.stringify(result.proof) });
