...
export class Indexer implements IndexerInterface {
  ...
  async getCensuringCount (blockHash: string, contractAddress: string, _whose: number): Promise<ValueResult> {
    const entity = await this._db.getGetCensuringCount({ blockHash, contractAddress, _whose });
    if (entity) {
      log('getCensuringCount: db hit.');
    ...
    const contractResult = await contract.getCensuringCount(_whose, { blockTag: blockHash });

    const value = ethers.BigNumber.from(contractResult).toBigInt();

    const result: ValueResult = { value };

    await this._db.saveGetCensuringCount({ blockHash, blockNumber, contractAddress, _whose, value: result.value, proof: JSONbigNative.stringify(result.proof) });
  ...
  async getCensuring (blockHash: string, contractAddress: string, _whose: number): Promise<ValueResult> {
    ...
    const contract = new ethers.Contract(contractAddress, abi, this._ethProvider);
    const contractResult = await contract.getCensuring(_whose, { blockTag: blockHash });
  ...
  async getCensuredByCount (blockHash: string, contractAddress: string, _who: number): Promise<ValueResult> {
    const entity = await this._db.getGetCensuredByCount({ blockHash, contractAddress, _who });
    if (entity) {
      log('getCensuredByCount: db hit.');
    ...
    const contract = new ethers.Contract(contractAddress, abi, this._ethProvider);
    const contractResult = await contract.getCensuredByCount(_who, { blockTag: blockHash });

    const value = ethers.BigNumber.from(contractResult).toBigInt();

    const result: ValueResult = { value };

    await this._db.saveGetCensuredByCount({ blockHash, blockNumber, contractAddress, _who, value: result.value, proof: JSONbigNative.stringify(result.proof) });
  ...
  async getCensuredBy (blockHash: string, contractAddress: string, _who: number): Promise<ValueResult> {
    ...
    const contract = new ethers.Contract(contractAddress, abi, this._ethProvider);
    const contractResult = await contract.getCensuredBy(_who, { blockTag: blockHash });
